...
@protocol VideoEnhanceDelegate;

@interface IJKFFMoviePlayerController : NSObject <IJKMediaPlayback>

- (id)initWithContentURL:(NSURL *)aUrl
             withOptions:(IJKFFOptions *)options
                 tracker:(id<IJKTrackerProtocol>)tracker
                  abTest:(id<IJKABTestProtocol>)abTest;
...
// video enhance
@property (atomic, assign) BOOL magicPixEnable;
@property (nonatomic, weak) id<VideoEnhanceDelegate> videoDelegate;
- (void) putVideoFrame:(int)width h:(int)height frame:(uint8_t*)frame len:(int)length;
...
// video enhance callback delegate
@protocol VideoEnhanceDelegate <NSObject>
- (void)didEnhanceVideoFrame:(int)width h:(int)height frame:(uint8_t*)frame len:(int)length;
@end
